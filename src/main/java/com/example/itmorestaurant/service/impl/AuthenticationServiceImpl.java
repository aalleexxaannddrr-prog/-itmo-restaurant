...
import com.example.itmorestaurant.enums.TokenType;
    ...
    public AuthenticationResponse register(RegisterRequest request) throws ParseException {
        ...
        if (!StringUtils.isEmpty(user.getEmail())) {
            String message = String.format(
                    "Здравствуйте, %s! \n" +
                            "Добро пожаловать в Itmo-Restaurant. Ваш код активации: %s",
                    user.getUsername(),
                    user.getActivationCode()
            );

            mailSender.send(user.getEmail(), "Код подтверждения аккаунта Itmo-Restaurant", message);
        }
